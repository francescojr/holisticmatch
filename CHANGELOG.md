...
- **Email Verification Gateway Field: `na_contencao`**
  ...
  **Implementation:**
  ...
  2. **Database Migration** (`backend/professionals/migrations/0007_add_na_contencao_field.py`)
     ...
     - Applied successfully to database: `python manage.py migrate`
     - Status: ✅ Deployed
  ...
  **Testing:**
  ...
  - Updated serializer tests to verify both `is_active` and `na_contencao` fields present
  - Fixture updates to handle new field in test data
  - Email verification tests passing with new field logic
